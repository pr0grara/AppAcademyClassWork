...
def richer_than_england
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
    gdp/population > (SELECT gdp/population AS per_capita_gdp
    FROM countries
...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    FROM 
      countries
    WHERE
      population > (SELECT population FROM countries WHERE name LIKE 'Canada')
      AND population < (SELECT population FROM countries WHERE name LIKE 'Poland')
  SQL
end
